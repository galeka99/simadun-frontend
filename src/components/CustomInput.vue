...
<template>
  <input v-if="!isPassword" :id="props.id" :type="props.type ?? 'text'" :class="`${props.class} form-input`" :autofocus="props.autofocus" v-model="model">
  <div v-if="isPassword" :class="`${props.class} relative`">
    <input :id="props.id" :type="showPassword ? 'text' : 'password'" class="form-input w-full" :autofocus="props.autofocus" v-model="model">
